...
%     fraction = 'test'; % Testing
    ...
    x 			= 1:dim_f0;
    momentum_f0 = zeros(size(src_f0_feat));
    ...
    for file_idx = 1:size(src_f0_feat,1)
        for sample = 1:N_samples
            ...
            if sum(y_s(:)==0)==length(y_s) || sum(y_t(:)==0)==length(y_t) 
                momentum_f0(file_idx, sample,:) = nan(1,dim_f0);
            else
                try
                    momentum_f0(file_idx, sample,:) = get_momentum(y_s, y_t, kx, ky)';
                catch
                    momentum_f0(file_idx, sample,:) = nan(1,dim_f0);
                end
            end
